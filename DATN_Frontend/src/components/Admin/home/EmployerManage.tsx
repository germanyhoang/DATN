...
const EmployerManage = () => {
  ...
  useEffect(() => {
    if (userEpr) {
      const mappedData = userEpr.map((item: any, index: any) => ({
        key: String(index),
        ...item,
      }));
      setData(mappedData); // Lưu dữ liệu vào state
    }
  }, [userEpr]);
  interface DataType {
    key: string;
  ...
  const handleBlock = async (user: any) => {
    ...
    if (result) {
      // Cập nhật state khi khóa tài khoản thành công
  ...
  const handleUnlock = async (user: any) => {
    ...
    if (result) {
      // Cập nhật state khi mở khóa tài khoản thành công
      const updatedData = data.map((item) => 
        ...
        <Space size="middle">
          {
            record.isBlock ?
              <Popconfirm
                title="Bạn chắc chắn muốn khóa tài khoản người dùng này?"
